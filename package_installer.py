...
from PyQt5.QtWidgets import (
    ...
    QListWidget, QMessageBox, QTextEdit, QFileDialog, QProgressBar,
...
# ✅ فرض التشغيل كـ root
if os.geteuid() != 0:
    print("This application requires root privileges. Relaunching with sudo...")
    os.execvp("sudo", ["sudo", sys.executable] + sys.argv)

# ✅ مسار الأيقونات المثبتة مع الحزمة
ICON_PATH = "/usr/share/package-installer/"
...
class PackageInstaller(QWidget):
    ...
    def initUI(self):
        ...
        self.setWindowIcon(QIcon(ICON_PATH + "arch_pkg.png"))
        ...
        self.solve_problem_button = QPushButton('Solve Problem')
        ...
        # ✅ أيقونات الأزرار
        self.install_button.setIcon(QIcon(ICON_PATH + "install_icon.png"))
        self.remove_button.setIcon(QIcon(ICON_PATH + "remove_icon.png"))
        self.browse_button.setIcon(QIcon(ICON_PATH + "browse_icon.png"))
        self.solve_problem_button.setIcon(QIcon(ICON_PATH + "solve_icon.png"))
        ...
        self.solve_problem_button.clicked.connect(self.solve_problem)
        ...
        layout.addWidget(QLabel('Terminal Output:', self))
    ...
    def install_remote_package(self, package_name):
        if package_name:
            command = ["pacman", "-S", package_name, "--noconfirm"]
            self.run_command(command)

    def install_local_package(self, package_path):
        if os.path.isfile(package_path):
            command = ["pacman", "-U", package_path, "--noconfirm"]
            self.run_command(command)
        else:
    ...
    def remove_package(self, package_name):
        if package_name:
            command = ["pacman", "-R", package_name, "--noconfirm"]
            self.run_command(command)
